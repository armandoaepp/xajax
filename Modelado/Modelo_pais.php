...
try{
	...
	$con=new Conexion();
	$consulta="Insert into pais values(NULL,'$this->PaisNombre')";
...
try{
	...
	if($this->getPaisNombre()!="")
		$parametro .="WHERE PaisNombre like '".$this->getPaisNombre()."%'";


	include_once 'conexion.php';
	$con=new Conexion();
	$consulta="Select pais.PaisId As 'PaisId',pais.PaisNombre As 'Pais'
	 from pais ".$parametro."  order by 1 desc";
...
try{
	include_once 'conexion.php';
	$con=new Conexion();
	$consulta="DELETE FROM pais WHERE PaisId='$this->PaisId'";
	$rpta=$con->MetodoRegistrar($consulta);
}catch(exception $e){
 	$rpta=$e->getMessage();}
	return $rpta;

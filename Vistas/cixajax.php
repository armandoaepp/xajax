...
class CixAJAX
{
    ...
    function ImprimeTableF($idTabla, $data, $pactu = "", $peli = "",$classCSS="table")
    {
        try {
            $tabla = '<table id="' . $idTabla . '" class="'.$classCSS.'" >';
            if (count($data["columnas"]) > 0) {
    ...
    // $crxpag=numero de filas a mostra, $paginap = pagina mostrar x defecto pagina 1, $cantlink = cantida de link de numerocion x defecto es 5
    //$orderby = ordenar por columnas x defecto es -1, $classCSS=nombre de la clases css para la tabla
                             // $idTabla, $data, $pactu, $peli, $crxpag, $paginap = 1, $cantlink = 5,$orderby = -1,$buscar="",$classCSS="",$link_td=""
    function ImprimeTablePOB($idTabla, $data, $pactu, $peli, $crxpag, $paginap = 1, $cantlink = 5, $orderby = -1, $buscar = "",$classCSS="table", $link_td = "")
    {
        $tabla = "";
        $tabla .= '<table id="' . $idTabla . '" class="'.$classCSS.'" >';
        try {
